<template>
  <div class="upload-page-container">
    ...
    >
      ...
      <!-- 上传状态 -->
      <el-row class="row-item" v-if="imgData.base64Url">
        <div class="upload-status">
          <div class="file-status">
            <div class="filename">{{ filename.now }}</div>

            <div
              class="upload-tips wait-upload"
      ...
      <!-- 外链 -->
      <el-row class="row-item" v-if="uploadStatus.progress === 100">
        <div class="external-link">
          <el-input
            ...
            readonly
          >
            <el-button slot="append" @click="copyLink('GitHub')"
              >复制GitHub外链
            </el-button>
          </el-input>
          <el-input
            ...
            readonly
          >
            <el-button slot="append" @click="copyLink('CDN')"
              >复制CDN外链
            </el-button>
          </el-input>
        </div>
        ...
        <div class="upload-tools">
          <div class="repos-dir-info" v-if="userConfigInfo.selectedRepos">
            <span class="repos-dir-info-item">
              仓库：<el-tag size="small">{{
                userConfigInfo.selectedRepos
              }}</el-tag>
            </span>
            <span
              class="repos-dir-info-item"
              v-if="userConfigInfo.selectedBranch"
            >
              分支：<el-tag size="small">{{
                userConfigInfo.selectedBranch
              }}</el-tag>
            </span>
            <span class="repos-dir-info-item" v-if="userConfigInfo.selectedDir">
              目录：<el-tag size="small">{{
          ...
          <UploadTools
            ...
            :is-show-rename="
              imgData.base64Url !== '' && uploadStatus.progress !== 100
            "
            :is-show-hash-name="
              imgData.base64Url !== '' && uploadStatus.progress !== 100
            "
            :is-show-transform-markdown="
              !uploadStatus.uploading && uploadStatus.progress === 100
            "
          />
        </div>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    uploadImage() {
      ...
      this.$axios
        .put(uploadUrlHandle(this.userConfigInfo, this.filename.now), data, {
          headers: {
    ...
    uploadedHandle(res) {
      ...
      // 生成外链
      this.externalLink.github = generateExternalLink(
        "github",
        res.data.content,
        this.userConfigInfo
      );
      this.externalLink.cdn = generateExternalLink(
        "cdn",
        res.data.content,
        this.userConfigInfo
      );
      this.externalLink.markdown_gh = generateExternalLink(
        "markdown_gh",
        res.data.content,
        this.userConfigInfo
      );
      this.externalLink.markdown_cdn = generateExternalLink(
        "markdown_cdn",
        res.data.content,
        this.userConfigInfo
      );

      this.externalLink.input_github = this.isTransformMarkdown
        ? this.externalLink.markdown_gh
        : this.externalLink.github;
      this.externalLink.input_cdn = this.isTransformMarkdown
        ? this.externalLink.markdown_cdn
        : this.externalLink.cdn;
    ...
    transformMarkdown(e) {
      ...
      this.externalLink.input_github = e
        ? this.externalLink.markdown_gh
        : this.externalLink.github;
      this.externalLink.input_cdn = e
        ? this.externalLink.markdown_cdn
        : this.externalLink.cdn;
    },
    ...
    getImage(url, file) {
      ...
      this.filename.now = this.isHashRename
        ? `${name}.${hash}.${suffix}`
        : file.name;
